---
name: define-goal
description: Define concrete, measurable goals before starting work. Invoke when user asks to set a goal, define an objective, clarify success criteria, or turn a fuzzy intention into a quantitative outcome.
---
...
> Reduce checkout API p95 latency below 250 ms for the documented slow path by making the smallest safe server-side change, then verify with the existing local latency benchmark showing p95 under 250 ms across 3 consecutive runs.
